...
$str_filtros = count($filtros) ?  where_filtros($filtros) : "WHERE ({$tabla}.id LIKE '%{$search}%' OR {$tabla}.NombreG LIKE '%{$search}%' OR {$tabla}.Nombre LIKE '%{$search}%')";
...
function where_filtros($filtros){
    ...
    return count($where) ? "WHERE ({$tabla}.id LIKE '%{$search}%' OR {$tabla}.NombreG LIKE '%{$search}%' OR {$tabla}.Nombre LIKE '%{$search}%') AND ".implode(' AND ',$where) : "";
}
