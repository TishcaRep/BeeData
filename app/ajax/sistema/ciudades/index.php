...
$str_filtros = count($filtros) ?  where_filtros($filtros) : "WHERE ({$tabla}.CodigoPostal LIKE '%{$search}%' OR {$tabla}.Colonia LIKE '%{$search}%' OR {$tabla}.Municipio LIKE '%{$search}%' OR {$tabla}.Estado LIKE '%{$search}%' )";
...
$query = "SELECT idCatalogoCiudad as id ,Estado,Municipio,Colonia,CodigoPostal FROM {$tabla} {$args_query}";
...
function where_filtros($filtros){
    ...
    return count($where) ? "WHERE ({$tabla}.CodigoPostal LIKE '%{$search}%' OR {$tabla}.Colonia LIKE '%{$search}%' OR {$tabla}.Municipio LIKE '%{$search}%' OR {$tabla}.Estado LIKE '%{$search}%' ) AND ".implode(' AND ',$where) : "";
}
