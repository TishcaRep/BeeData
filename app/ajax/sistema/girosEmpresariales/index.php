...
$str_filtros = count($filtros) ?  where_filtros($filtros) : "WHERE ({$tabla}.Nombre LIKE '%{$search}%' OR {$tabla}.idGirosEmpresariales LIKE '%{$search}%')";
...
$query = "SELECT idGirosEmpresariales as id, Nombre FROM {$tabla} {$args_query}";
...
function where_filtros($filtros){
    ...
    return count($where) ? "WHERE ({$tabla}.Nombre LIKE '%{$search}%' OR {$tabla}.idGirosEmpresariales LIKE '%{$search}%') AND ".implode(' AND ',$where) : "";
}
