...
$str_filtros = count($filtros) ?  where_filtros($filtros) : "AGRUPBY ";
$args_query = $str_filtros;
$args_query .= (count($orderby)) ? " ORDER BY ".implode(",",$orderby)." GROUP BY(id)" : "GROUP BY(id)";
$args_query_tot = $args_query;
$args_query .= (($muestra>0) ? " LIMIT {$empieza},{$termina}" : "");
$query = "SELECT `id`, `Nombre`, COUNT(`idRes`) AS Numero_respuestas FROM {$tabla} {$args_query}";
...
function where_filtros($filtros){
    ...
    return count($where) ? "WHERE ({$tabla}.NombreGrupo LIKE '%{$search}%' OR {$tabla}.idGrupoRespuestas LIKE '%{$search}%') AND ".implode(' AND ',$where) : "";
}
